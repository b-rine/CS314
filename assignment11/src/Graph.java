...
public class Graph {
    ...
    public void dijkstra(String startName) {
        handleDijkstraPrecons(startName);
        PriorityQueue<Path> pq = prepDijkstra(startName);
        while (!pq.isEmpty()) {

            // get next prioritized path for current vertex
            Path currentPath = pq.poll();
            Vertex currentVertex = currentPath.dest;

            // only look at unvisited vertices
            if (currentVertex.scratch == 0) {
                currentVertex.scratch = 1;
                for (Edge e : currentVertex.adjacent) {
                    Vertex neighbor = e.dest;
                    double neighborWeight = neighbor.weightedCostFromStartVertex;
                    double currentWeight = currentVertex.weightedCostFromStartVertex;

                    // updating for new paths that cost less
                    // than previously checked paths
                    if (currentWeight + e.cost < neighborWeight) {
                        neighbor.weightedCostFromStartVertex = currentWeight + e.cost;
                        neighbor.numEdgesFromStartVertex =
                            currentVertex.numEdgesFromStartVertex + 1;

                        // connect vertices, then eventually look at that neighbor's
                        // surrounding paths
                        neighbor.prev = currentVertex;
                        pq.add(new Path(neighbor, neighbor.weightedCostFromStartVertex));
                    }
                }
            }
        }
    ...
    // reset vertices, set up priority queue and instance variables for dijkstra
    // algorithm from startName vertex to every other vertex that is connected
    private PriorityQueue<Path> prepDijkstra(String startName) {
        currentStartVertexName = startName;
        clearAll();
        Vertex start = vertices.get(startName);
        start.weightedCostFromStartVertex = 0;
        start.numEdgesFromStartVertex = 0;
        PriorityQueue<Path> result = new PriorityQueue<>();
        result.add(new Path(start, start.weightedCostFromStartVertex));
        return result;
    }

    // check preconditions for dijkstra
    private void handleDijkstraPrecons(String startName) {
        if (startName == null) {
            throw new IllegalArgumentException("Violation of precondition. " +
                "Vertex name may not be null.");
        }
    ...
    public void findAllPaths(boolean weighted) {
        // reset path info to avoid overlap from multiple calls
        for (Vertex vert : vertices.values()) {
            vert.clearPathInfo();
        }

        // must start at all vertices to find every path, but change algorithm
        // as according to parameter
        for (String startName : vertices.keySet()) {
            ...
            for (String otherName : vertices.keySet()) {
                ...
                // make sure other vertices are connected to start before updating
                // vertex variables
                if (!otherName.equals(startName) && !findPath(otherName).isEmpty()) {
                    Path connection = getPath(otherName);
                    startVertex.numVertexConnected++;
                    startVertex.totalUnweightedPathLength += getNumEdgesFromStart(otherName);
                    startVertex.totalWeightedPathLength += weighted ?
                        getWeightedCostFromStart(otherName) : getNumEdgesFromStart(otherName);
                    if (longest == null ||
                        longest.weightedCostOfPath < connection.weightedCostOfPath) {
                        longest = connection;
                    }
                }
            }
    ...
    public int getNumEdgesFromStart(String dest) {
        if (currentStartVertexName == null) {
            throw new IllegalStateException("method " +
                            "findUnweightedShortestPath or dijkstra must be " +
     ...
     * findUnweightedShortestPath or dijkstra.
     ...
     * been set by calling findUnweightedShortestPath or dijkstra.
     * @param destName The destination vertex.
     */
     ...
     * <br>pre: findAllPaths has been called.
     * @return the names of the vertices in a path equal to the diameter in this graph. If there
    ...
    private static class Vertex {
        ...
        // These instance variables are set in findUnweightedShortestPath
